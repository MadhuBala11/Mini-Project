from flask import *
import mysql.connector, random, string, base64, pickle
import pandas as pd
import numpy as np
...
def yoga2():
    ...
    if request.method == "POST":
        ...
        if results.pose_landmarks:
            ...
            query = "INSERT INTO dashboard (name, email, mood, yoga, uploaded_img, corrected_img, feedback, date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
            values = (user_name, user_email, mood, pose_name, uploaded_img_binary_data, corrected_img_binary_data, str(prd_result), current_date)
            executionquery(query, values)
